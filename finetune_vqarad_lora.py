...
def create_ans2label(dataset_train, dataset_test, vocab):
   ...
   index_to_answer = {index: answer for index, answer in enumerate(unique_answers)}
   ...
   return unique_answers, answer_to_index, predicted_answers, index_to_answer
...
unique_answers, answer_to_index, predicted_answers, index_to_answer = create_ans2label(train_data, test_data, vocab)
...
model = ViltForQuestionAnswering.from_pretrained("dandelin/vilt-b32-mlm", id2label=answer_to_index, label2id=index_to_answer)
...
optimizer = torch.optim.AdamW(lora_model.parameters(), lr=0.001)
